...
from algorithms.hifno_aug import HiFNOAgent
...
def main(args):
	...
	# Create working directory(仅在主进程进行输出、创建目录等, 避免多进程重复)
	if dist.get_rank() == 0:
	...
	for step in range(start_step, args.train_steps+1):
		if done:
			...
			# Save agent periodically(也只在 rank=0 进行)
